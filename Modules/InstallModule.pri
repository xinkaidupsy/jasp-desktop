...
isEmpty(MODULE_NAME) {
	...
	unix{ ################## Unix ##################
		...
		for(LANGUAGE_CODE, SUPPORTED_LANGUAGES) {
			...
			GenerateLanguageFiles$${MODULE_NAME}.commands += lupdate -locations none -extensions cpp,qml -recursive $$PWD/$${MODULE_NAME} -ts $$PWD/$${MODULE_NAME}/po/QML-$${LANGUAGE_CODE}.po ;				$$escape_expand(\\n\\t)
			GenerateLanguageFiles$${MODULE_NAME}.commands += msgattrib --no-obsolete --no-location $$PWD/$${MODULE_NAME}/po/QML-$${LANGUAGE_CODE}.po -o $$PWD/$${MODULE_NAME}/po/QML-$${LANGUAGE_CODE}.po ;		$$escape_expand(\\n\\t)

			#Create $${MODULE_NAME}.qm
			GenerateLanguageFiles$${MODULE_NAME}.commands += lrelease $$PWD/$${MODULE_NAME}/po/QML-$${LANGUAGE_CODE}.po -qm $${QM_FILE_LOCATION}/$${MODULE_NAME}-$${LANGUAGE_CODE}.qm ;							$$escape_expand(\\n\\t)
			}
